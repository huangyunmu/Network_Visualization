...
public class NodeClickListener implements ViewerListener, MouseInputListener {
	 ...
	 * 
	 * @param vpipe
	 *            - Viewer Pipe of the graph UI
	 * @param view
	 *            - View of the current graph in swing
	 * @param graph
	 *            - graph object for the current graph in use
	 */
	public NodeClickListener(ViewerPipe vpipe, View view, Graph graph, GraphManager manager) {
		this.loop = true;
		this.vpipe = vpipe;
		this.view = view;
		this.graph = graph;
	 ...
	 * 
	 * @param id
	 *            - not used, but inherited by interface
	 */
	...
	public void buttonPushed(String id) {
		...
		// double pos[]=manager.getDrawer().getNodePosition(id);
		// System.out.printf("%f %f %f", pos[0],pos[1],pos[2]);
		manager.controlNodeComment(id);
	 ...
	 * 
	 * @param n
	 *            - node to label adjacent edges of
	 */
	 ...
	 * 
	 * @param n
	 *            - the node to apply to adjacent edges
	 */
	private void unlabelAdjacentEdges(Node n) {
		for (Edge e : n.getEdgeSet()) {
			...
			if ((ui_label_v1 == null || ui_label_v1.equals("")) && (ui_label_v2 == null || ui_label_v2.equals(""))) {
